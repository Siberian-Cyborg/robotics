...
for i=1:length(rob.kl)
    ...
    %Impulsaenderung - Schwerkraft
        F = rob.kl(i).m * rob.kl(i).Bi_ddot_r_s ...
            - rob.kl(i).m * rob.kl(i).A_i0 * rob.B0_g;
    
    %Drallaenderung - Moment der Schwerkraft
        T = rob.kl(i).I_o * rob.kl(i).Bi_dot_omega + tilde(rob.kl(i).Bi_omega) * rob.kl(i).I_o * rob.kl(i).Bi_omega ...
    ...
    %Projektion auf zwangsfreie Richtungen und Addition zu tau_id
        rob.tau_id = rob.tau_id + rob.kl(i).Bi_Jt_o' * F + rob.kl(i).Bi_Jr' * T;
end
